...
export const ResponsiveGrid: React.FC<{
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ${gapMap[gap]} ${className}`}>
      {children}
    </div>
  );
};
...
export const AdminDashboardGrid: React.FC<{
  ...
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4 md:gap-6 lg:gap-8 ${className}`}>
      {children}
    </div>
  );
};
...
export const DashboardTile: React.FC<{
  ...
  return (
    ...
    >
      ...
      {value && <p className="text-lg md:text-2xl font-bold text-gray-900 mt-2">{value}</p>}
      {children}
    </button>
  );
};
  ...
  return (
    <>
      ...
      <div className="lg:hidden space-y-3">
        {rows.map((row, rowIdx) => (
          <div key={rowIdx} className="bg-white p-4 rounded-lg border border-gray-200">
            {row.map((cell, cellIdx) => (
              <div key={cellIdx} className="flex justify-between py-2 text-sm">
                <span className="font-semibold text-gray-700">{headers[cellIdx]}</span>
...
export const ResponsiveFlex: React.FC<{
  ...
  return (
    <div className={`flex flex-col md:flex-row ${gapMap[gap]} ${alignMap[align]} ${className}`}>
      {children}
    </div>
  );
};
...
export const ResponsiveSection: React.FC<{
  ...
  return (
    <section className={`py-8 md:py-12 lg:py-16 px-4 md:px-6 lg:px-8 ${className}`}>
      {title && (
