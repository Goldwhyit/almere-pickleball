    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <ResponsiveContainer>
          <div className="space-y-4 md:space-y-6 lg:space-y-8">
            ...
            <p className="text-base md:text-lg lg:text-xl max-w-2xl text-blue-100">
              This page demonstrates all responsive patterns. Current breakpoint: <strong>{width}px</strong>
            </p>
          ...
          <ResponsiveGrid>
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition">
                ...
                <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-2">Feature {i}</h3>
                <p className="text-gray-600 text-sm md:text-base">
                  Responsive layout automatically adjusts from 1 column on mobile to 3 columns on desktop.
                </p>
              </div>
            ))}
          </ResponsiveGrid>
        ...
        <ResponsiveContainer>
          <div className="space-y-8">
            <div>
              ...
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900">H1: 3xl → 5xl</h1>
            </div>

            <div>
              <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900">H2: 2xl → 4xl</h2>
            </div>

            <div>
              <h3 className="text-xl md:text-2xl lg:text-3xl font-semibold text-gray-800">H3: xl → 3xl</h3>
            </div>

            <div>
              <p className="text-base md:text-lg lg:text-xl text-gray-700 leading-relaxed">
                Body text scales from base to lg. Perfect for maintaining readability at all screen sizes.
              </p>
            </div>

            <div>
              <p className="text-xs md:text-sm lg:text-base text-gray-600">Small text: xs → base</p>
            </div>
          </div>
        </ResponsiveContainer>
      </ResponsiveSection>
      ...
      <ResponsiveSection title="Admin Dashboard">
        <ResponsiveContainer size="wide">
          <AdminDashboardGrid>
            <DashboardTile title="Total Users" value="1,234" color="blue" icon="👥" />
            <DashboardTile title="Revenue" value="€12,345" color="green" icon="💰" />
            <DashboardTile title="Tournaments" value="24" color="purple" icon="🏓" />
      ...
      <ResponsiveSection title="Device Detection" className="bg-blue-50">
        <ResponsiveContainer>
          <div className="space-y-4">
            {isMobile && (
              <div className="p-4 bg-blue-200 text-blue-900 rounded-lg">📱 You're on Mobile ({width}px)</div>
            )}
            {isTablet && (
              <div className="p-4 bg-purple-200 text-purple-900 rounded-lg">📱 You're on Tablet ({width}px)</div>
            )}
            {isDesktop && (
              <div className="p-4 bg-green-200 text-green-900 rounded-lg">💻 You're on Desktop ({width}px)</div>
            )}
          </div>
        </ResponsiveContainer>
      </ResponsiveSection>
      ...
      <ResponsiveSection className="bg-gradient-to-r from-blue-600 to-blue-700 text-white">
        <ResponsiveContainer>
          <div className="text-center space-y-6">
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold">Ready to Build Responsive?</h2>
            <p className="text-base md:text-lg text-blue-100 max-w-2xl mx-auto">
              Use this template and the responsive components to build your pages. Everything adapts perfectly!
            </p>
