...
import type { AdminStats, AdminMember } from './useAdminData';
...
const DashboardKPIs: React.FC<DashboardKPIsProps> = ({ stats, loading }) => {
  ...
  const conversionPct = trialCount > 0 ? Math.round((conversionCount / trialCount) * 100) : 0;

  // Skeleton placeholder
  const SkeletonTile = () => (
    <div className={`bg-white rounded-lg shadow-md p-6 flex flex-col justify-between items-start ${TILE_HEIGHT}`}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
      {loading
        ...
        : kpiConfig.map((kpi) => (
            ...
            >
              <span className="text-3xl mb-2" aria-hidden>{kpi.icon}</span>
              <p className="text-sm text-gray-600 mb-1">{kpi.label}</p>
              <p className="text-3xl font-bold text-gray-900">{kpiValues[kpi.key]}</p>
            </div>
          ))}
    </div>
  );
